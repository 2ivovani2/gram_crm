...
type IconName = "home" | "bot" | "flow" | "chart" | "more" | "arrow";
...
function MiniApp() {
  ...
  return <div className="app-shell mini-app">
    ...
    <main className="content">{error&&<ErrorBox message={error}/>}
      {tab==="Обзор"&&<><section className="hero-panel wet-panel"><div><span className="eyebrow">СЕЙЧАС В GRAMLY</span><h1>Ваши приветствия работают.</h1><p>Мы принимаем события Telegram, выбираем сценарий и доставляем сообщение каждому новому участнику.</p><button className="button acid quick-action" onClick={()=>openTab("Цепочки")}>Открыть цепочки <Icon name="arrow"/></button></div><div className="route"><span>СОБЫТИЕ</span><i/><span>GRAMLY</span><i/><span>ДОСТАВЛЕНО</span></div></section><section className="metrics wet-panel"><Metric label="Боты" value={dashboard?.bots??0}/><Metric label="Каналы" value={dashboard?.channels??0}/><Metric label="Контакты" value={dashboard?.contacts??0}/><Metric label="Доставлено" value={dashboard?.delivered??0}/></section></>}
      {tab==="Боты"&&<section><div className="section-head"><div><span className="eyebrow">ПОДКЛЮЧЕНИЯ</span><h2>Мои боты</h2></div></div><div className="bot-list">{bots.map(bot=><article className="wet-panel" key={bot.id}><div className="bot-id"><span className="avatar">{bot.display_name.slice(0,1)}</span><div><strong>@{bot.username||bot.display_name}</strong><small>{bot.channels} каналов · {bot.contacts} контактов</small></div></div><Status ok={bot.webhook_configured}>{bot.webhook_configured?"Webhook online":"Нужно подключить"}</Status><button className="button ghost" onClick={()=>{setActiveBot(bot.id);openTab("Цепочки")}}>Цепочки <Icon name="arrow"/></button></article>)}</div></section>}
      {tab==="Цепочки"&&<section><div className="section-head"><div><span className="eyebrow">КОНТЕНТ</span><h2>Цепочки</h2></div><select value={activeBot||""} onChange={e=>setActiveBot(Number(e.target.value))}>{bots.map(bot=><option key={bot.id} value={bot.id}>@{bot.username}</option>)}</select></div><div className="flow-list">{flows.map(flow=><button className="wet-panel" key={flow.id} onClick={()=>setEditor(flow)}><div><span>{flow.kind==="farewell"?"ПРОЩАНИЕ":"ПРИВЕТСТВИЕ"}</span><strong>{flow.name}</strong></div><div><small>{flow.versions.find(v=>v.status==="published")?"Опубликовано":"Черновик"}</small><Icon name="arrow"/></div></button>)}</div></section>}
      {tab==="Аналитика"&&<section><div className="section-head"><div><span className="eyebrow">ТОЛЬКО РЕАЛЬНЫЕ ДАННЫЕ</span><h2>Аналитика</h2></div></div><div className="metrics wet-panel"><Metric label="Успешные цепочки" value={analytics?.deliveries?.completed||0}/><Metric label="Ошибки" value={(analytics?.deliveries?.failed||0)+(analytics?.deliveries?.partial||0)}/><Metric label="Показы ротации" value={analytics?.rotation?.impressions||0}/><Metric label="Подписки" value={analytics?.rotation?.conversions||0}/></div><div className="note wet-panel">Telegram не передаёт пол и страну пользователя — поэтому мы не показываем выдуманные данные.</div></section>}
      ...
      {tab==="Подписка"&&<section><button className="back-link" onClick={()=>openTab("Ещё")}>← Назад</button><div className="section-head"><div><span className="eyebrow">FREE / BUSINESS</span><h2>Тариф</h2></div></div><div className="plan wet-panel"><div><Status ok={me?.access?.plan==="business"}>{me?.access?.plan_name}</Status><h3>{me?.access?.plan==="business"?"Без рекламы. Ротация включена.":"Полный функционал с рекламой Gramly."}</h3></div>{plans.filter(p=>p.slug==="business").map(plan=><div className="price" key={plan.id}><strong>{plan.price_rub?`${plan.price_rub} ₽`:"Цена настраивается"}</strong><span>30 дней</span></div>)}</div></section>}
      {tab==="Партнёры"&&<section><button className="back-link" onClick={()=>openTab("Ещё")}>← Назад</button><div className="section-head"><div><span className="eyebrow">ПАРТНЁРСКИЙ СЧЁТ</span><h2>Партнёрская программа</h2></div></div><div className="metrics wet-panel"><Metric label="Активные клиенты" value={partners?.active_referrals||0}/><Metric label="Доступно" value={`${partners?.balance_rub||0} ₽`}/></div><label className="copy-field wet-panel"><span>Ваша ссылка</span><input readOnly value={partners?.url||""}/><button onClick={()=>navigator.clipboard.writeText(partners?.url||"")}>Копировать</button></label></section>}
    </main>
