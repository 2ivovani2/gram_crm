...
## 2026-05-12 — Project reorganization: unified compose + subdomain split

**Agent:** Claude (automated refactoring session)

### What changed

- **Unified compose:** создан `docker-compose.yml` в корне `full_gramly/` — объединяет `spambotcontrol` и `autosending` в единый стек
- **Multi-domain nginx:** `nginx/gramly.conf` обслуживает `gramly.tech` / `crm.gramly.tech` / `spam.gramly.tech`
- **autosending → spam.gramly.tech:** autosending встроен в единый стек, назначен поддомен
- **One PostgreSQL:** `autosending` БД создаётся в том же PostgreSQL контейнере через `scripts/pg_init.sh`
- **Удалены /stats/ routes:** `StatsDashboardView`, `ClientsView` — ручной контроль воркеров больше не нужен
- **Удалены stats templates:** `stats_dashboard.html`, `stats_clients.html`
- **Landing обновлён:** ссылки на `crm.gramly.tech` и `spam.gramly.tech`
- **Bot упрощён:** удалены handlers для broadcasts, stats, clients, applications, withdrawals, curator, worker management, join_request, subscription. Бот теперь показывает CRM и docs ссылки
- **autosending DB:** `DATABASE_URL` читается из env var (не хардкод). SQLite по умолчанию, PostgreSQL в prod через compose

### Файлы в full_gramly/ (корень)
- `docker-compose.yml` — unified compose (новый)
- `nginx/gramly.conf` — multi-domain nginx (новый)
- `.env.example` — root env example (новый)
- `Makefile` — root makefile (новый)
- `scripts/pg_init.sh` — postgres multi-db init (новый)
- `CHANGELOG_AGENT.md` — полный список изменений (новый)

---

## 2026-05-06 — CRM entry forms: надёжное предзаполнение + убрана кнопка «Перейти»

### Проблемы

**Проблема 1 — пустая форма при открытии уже заполненного дня**

Шаблоны `entry_finance.html` и `entry_applications.html` использовали `{{ form.field.value|default:'X' }}` для рендера значений полей. Django's `BoundField.value()` в сочетании с `|default` фильтром ненадёжно работает для числовых полей: `Decimal('0.00')` является falsy в Python, из-за чего нулевые значения не всегда отображались из реального объекта, а при некоторых edge-кейсах (кэш шаблонов, особенности вызова метода через Django template engine) форма могла показывать пустые значения.

**Проблема 2 — лишняя кнопка «Перейти»**

Для смены даты требовалось: выбрать дату в `<input type="date">` → нажать кнопку «Перейти». Двойное действие без необходимости.

### Что исправлено

#### `templates/crm/entry_finance.html` и `templates/crm/entry_applications.html`

**Предзаполнение полей** — заменена ненадёжная `{{ form.field.value|default:'...' }}` на явную трёхсостояниую логику:
```
{% if form.is_bound %}
  {{ form.field.value }}          {# POST с ошибкой валидации → показываем введённое #}
{% elif existing %}
  {{ existing.field_name }}       {# GET с существующей записью → читаем напрямую из ORM объекта #}
{% else %}
  0.00                            {# GET без записи → пустая новая форма #}
{% endif %}
```

Это гарантированно надёжно:
- При успешном GET с существующей записью: читаем напрямую из `existing` (FinanceEntry / ApplicationEntry)
- При POST с ошибкой валидации: форма bound → `form.field.value` показывает введённое пользователем
- При новой записи (нет existing): показываем значение по умолчанию

**Убрана кнопка «Перейти»** — добавлен `onchange="goToDate()"` к `<input type="date">`. Навигация происходит мгновенно при выборе даты, без промежуточного клика.

### Как теперь работает flow

1. Пользователь открывает `/crm/entry/finance/` → форма для **сегодняшней** даты
   - Если данные уже внесены → форма **предзаполнена** значениями из БД, бейдж «Редактирование»
   - Если данных нет → пустая форма, бейдж «Новая запись»
2. Пользователь меняет дату в пикере → навигация **мгновенная** (без кнопки)
3. Открытый день уже заполнен → форма предзаполнена
4. Открытый день не заполнен → пустая форма

### Файлы
`templates/crm/entry_finance.html`, `templates/crm/entry_applications.html`
